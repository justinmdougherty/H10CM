...
export const RBACProvider: React.FC<RBACProviderProps> = ({ children }) => {
  ...
  const loginWithCertificate = async (): Promise<boolean> => {
    try {
      ...
      console.log('Attempting certificate authentication...');

      // For development, auto-login as admin if certificate is detected
      const adminUser = mockUsers.find((u) => u.has_certificate && u.role === 'Admin');
      if (adminUser) {
        setCurrentUser(adminUser);
        setIsAuthenticated(true);
        localStorage.setItem('tf_current_user', JSON.stringify(adminUser));
        return true;
      }
