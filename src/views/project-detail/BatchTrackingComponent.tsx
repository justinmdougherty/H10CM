...
const BatchTrackingComponent: React.FC<BatchTrackingComponentProps> = ({ project, steps }) => {
  ...
  const renderTableCellContent = (unit: ProductionUnit, column: TableColumnConfig) => {
    ...
    if (attribute) {
      ...
      return (
        <TableCell sx={{ fontWeight: attribute.attribute_name.includes('S/N') ? 500 : 400 }}>
          {attributeValue || '—'}
        </TableCell>
      );
    }
    ...
    <Paper sx={{ p: 3, width: '100%' }}>
                ...
                {currentVisibleUnits
                  ...
                  .map((unit) => {
                    const currentStepStatus =
                      unit.step_statuses?.find(
                        (ss) => ss.stepId.toString() === currentStepIdToUpdate,
                      )?.status || 'N/A';
                    return (
                      <Typography
                        key={unit.item_id}
                      ...
                      >
                        {unit.unit_serial_number || `Item ${unit.item_id}`}: {currentStepStatus}
                      </Typography>
                    );
                  })}
      ...
      >
        ...
        >
          <Box>
            Unit Details:{' '}
            {selectedUnitForDetail?.unit_serial_number || `Item ${selectedUnitForDetail?.item_id}`}
          ...
          {selectedUnitForDetail && (
            <Box>
              <Box sx={{ mb: 3 }}>
                ...
                {/* Display the primary identifier (first attribute) */}
                {(() => {
                  const primaryAttr = projectAttributes?.find((attr) => attr.display_order === 1);
                  ...
                  return (
                    <Typography variant="body1">
                      <strong>{primaryAttr?.attribute_name || 'Primary ID'}:</strong>{' '}
                      {primaryValue ||
                ...
                {/* Display the secondary identifier (second attribute) if it exists */}
                {(() => {
                  const secondaryAttr = projectAttributes?.find((attr) => attr.display_order === 2);
                  const secondaryValue = secondaryAttr
                    ? selectedUnitForDetail.attributes?.find(
                        (a) => a.attribute_definition_id === secondaryAttr.attribute_definition_id,
                      )?.attribute_value
                    : null;

                  return secondaryValue ? (
                    <Typography variant="body1">
                      <strong>{secondaryAttr?.attribute_name}:</strong> {secondaryValue}
                    </Typography>
                  ) : null;
                })()}
                <Typography variant="body1">
                ...
                {projectAttributes?.map((attr) => {
                  ...
                  // Handle special cases for Unit S/N and PCB S/N which are in the main unit object
                  if (attr.attribute_name === 'Unit S/N') {
                    attributeValue = selectedUnitForDetail.unit_serial_number || '';
                  } else if (attr.attribute_name === 'PCB S/N') {
                    attributeValue = selectedUnitForDetail.pcb_serial_number || '';
                  } else if (selectedUnitForDetail.attributes) {
                  ...
                  // Only show attributes that are not the primary or secondary identifiers (since they're already shown above)
                  // or show all if they have values
                  return attributeValue && attr.display_order > 2 ? (
